...
int main(int argc, char** argv)
{
	...
	if (test_choise[T_LINE] == '1') {
		...
		//hdc_line(100, 100, 500, 110, 5, RGB(rand() % 256, rand() % 256, rand() % 256));
		//hdc_line(100, 100, 500, 500, 5, RGB(rand() % 256, rand() % 256, rand() % 256));
		//system("pause");
		...
		for (i = 0; i < 20; i++) {
			...
			/* 线的粗细 1~5，颜色随机 */
			hdc_line(x1, y1, x2, y2, 1 + rand() % 5, RGB(rand() % 256, rand() % 256, rand() % 256));
			Sleep(1);
		}
	...
	if (test_choise[T_MATHCURVE] == '1') {
		...
		struct {
			...
			{2053620, "张润泽", hdc_draw_Mathematical_curve_2053620},
			{2152046, "程瑞真", hdc_draw_Mathematical_curve_2152046},
			{0,NULL,NULL}
		};
	...
	if (test_choise[T_CUSTOM] == '1') {
		...
		struct {
			...
			{2053620, "张润泽", hdc_draw_cartoon_2053620, 400, 400},
			{2152046, "程瑞真", hdc_draw_cartoon_2152046, 400, 400},
			{0,NULL,NULL}
		};
		/* 显示显示助教哥哥/姐姐们画的卡通图像
		   注：调用函数前，先要添加对应的cpp并在.h中加函数声明
